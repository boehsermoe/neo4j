...
class Migration extends Component implements MigrationInterface
{
	...
	{
		echo "    > insert into $label ...";
		$time = microtime(true);
		$this->db->createCommand()->insert($label, $properties)->execute();
		echo " done (time: " . sprintf('%.3f', microtime(true) - $time) . "s)\n";
	}
	 ...
	 */
	public function createIndex($label, $property, $unique = false)
	{
		echo "    > create" . ($unique ? ' unique' : '') . " index $label (" . implode(',', (array) $property) . ") ...";
		$time = microtime(true);
		$this->db->createCommand()->createIndex($label, $property, $unique)->execute();
		echo " done (time: " . sprintf('%.3f', microtime(true) - $time) . "s)\n";
	}
	...
	{
		echo "    > drop index $label($property) ...";
		$time = microtime(true);
		$this->db->createCommand()->dropIndex($label, $property)->execute();
		echo " done (time: " . sprintf('%.3f', microtime(true) - $time) . "s)\n";
	}
}
